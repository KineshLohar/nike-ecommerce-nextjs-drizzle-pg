...
export default function Filters() {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            setOpen(false);
        }, 0); // Immediate but still asynchronous.
    ...
    return (
        <>
            ...
            {/* <aside className="sticky top-20 hidden h-fit min-w-60 rounded-lg border border-light-300 bg-light-100 p-4 md:block mb-20"> */}
  ...
  "
            >
                ...
                <Group title={`Gender ${activeCounts.gender ? `(${activeCounts.gender})` : ""}`} k="gender" setExpanded={setExpanded} expanded={expanded}>
                ...
                <Group title={`Size ${activeCounts.size ? `(${activeCounts.size})` : ""}`} k="size" setExpanded={setExpanded} expanded={expanded}>
                ...
                <Group title={`Color ${activeCounts.color ? `(${activeCounts.color})` : ""}`} k="color" setExpanded={setExpanded} expanded={expanded}>
                ...
                <Group title={`Price ${activeCounts.price ? `(${activeCounts.price})` : ""}`} k="price" setExpanded={setExpanded} expanded={expanded}>
                    <ul className="space-y-2">
                        {PRICES.map((p) => {
                ...
                <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true">
                    ...
                    <div className="absolute inset-y-0 left-0 w-80 max-w-[80%] overflow-auto bg-light-100 p-4 shadow-xl">
                        ...
                        <div className="md:hidden">
                            <Group title="Gender" k="gender" setExpanded={setExpanded} expanded={expanded}>
                                <ul className="space-y-2">
                                    {GENDERS.map((g) => {
                                        const checked = getArrayParam(search, "gender").includes(g);
                                        return (
                            ...
                            <Group title="Size" k="size" setExpanded={setExpanded} expanded={expanded}>
                            ...
                            <Group title="Color" k="color" setExpanded={setExpanded} expanded={expanded}>
                                <ul className="grid grid-cols-2 gap-2">
                                    {COLORS.map((c) => {
                                        const checked = getArrayParam(search, "color").includes(c);
                                        return (
                            ...
                            <Group title="Price" k="price" setExpanded={setExpanded} expanded={expanded}>
                                <ul className="space-y-2">
                                    {PRICES.map((p) => {
                                        const checked = getArrayParam(search, "price").includes(p.id);
                                        return (
