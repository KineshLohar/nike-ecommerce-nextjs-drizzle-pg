...
export async function getAllProducts(filters: NormalizedProductFilters): Promise<GetAllProductsResult> {
  ...
  if (hasPrice) {
    ...
    if (filters?.priceRanges?.length) {
      for (const [min, max] of filters?.priceRanges) {
        ...
        if (max !== undefined) {
          subConds.push(sql`(${variants.price})::numeric <= ${max}`);
        }
    ...
    if (filters.priceMin !== undefined || filters.priceMax !== undefined) {
      const subConds: SQL[] = [];
      if (filters.priceMin !== undefined) subConds.push(sql`(${variants.price})::numeric >= ${filters.priceMin}`);
      if (filters.priceMax !== undefined) subConds.push(sql`(${variants.price})::numeric <= ${filters.priceMax}`);
      if (subConds.length) priceBounds.push(and(...subConds)!);
    }
  ...
  const variantJoin = db
    .select({
      ...
      price: sql<number>`${variants.price}::numeric`.as("price"),
      colorId: variants.colorId,
      sizeId: variants.sizeId,
    })
    .from(variants)
  ...
  const imagesJoin = hasColor
    ? db
        ...
        .from(productImages)
        .innerJoin(variants, eq(variants.id, productImages.variantId))
        .where(
          inArray(
            variants.colorId,
...
export async function getProduct(productId: string): Promise<FullProduct | null> {
  const rows = await db
    .select({
      ...
      variantPrice: sql<number | null>`${variants.price}::numeric`,
      variantSalePrice: sql<number | null>`${variants.salePrice}::numeric`,
      variantColorId: variants.colorId,
      variantSizeId: variants.sizeId,
      variantInStock: variants.inStock,
    ...
    .leftJoin(colors, eq(colors.id, variants.colorId))
    .leftJoin(sizes, eq(sizes.id, variants.sizeId))
